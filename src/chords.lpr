...
function NoteInChord(const Key, Note: TSemitone; const OpenString: Boolean; const ChordType: TChordType): string;
var
  interval: Byte;
begin
  ...
  if interval in CHORD_SEMITONES[ChordType] then
    case ChordType of
      ctMajor7, ctDominant:   if (Key in [2, 7, 9, 12]) then Exit(FLATS[note]) else Exit(SHARPS[note]);
      ctMinor7, ctDiminished: if (Key in [2, 4, 9, 11]) then Exit(FLATS[note]) else Exit(SHARPS[note]);
    end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            row := row + NoteInChord(rootNote, note, finger = 0, chord);
          end;
        end;
        WriteLn(row);
      end;
